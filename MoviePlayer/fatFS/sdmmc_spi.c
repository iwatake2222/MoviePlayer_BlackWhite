...
#include "fatFSConfig.h"
#ifdef FATFS_MCC_USE_SIO
...
#define CS_LOW()	FATFS_MMC_CS_PORT &=~(1<<FATFS_MMC_CS_BIT)			/* CS=low */
#define	CS_HIGH()	FATFS_MMC_CS_PORT |= (1<<FATFS_MMC_CS_BIT)			/* CS=high */
...
void power_on (void)
{
	FATFS_MMC_CS_DDR |= 1 << FATFS_MMC_CS_BIT;
	CS_HIGH();
	SPI_OPEN_PRM prm;
